...
#class gen_drng:
    ...
    def tp_read(self,addr,quiet=0):
        if (addr > 0x602):
            ...
            #theresult = itp.ucrb(0,0x4e10)
            theresult = self.read_reg(self.addr_tpout) &0xffff
            #itp.ucrb(0,0x4e08,thecommand)
            #self.write_reg(self.addr_tpin,thecommand)
            #theresult = itp.ucrb(0,0x4e10)
            theresult = self.read_reg(self.addr_tpout) &0xffff
        if (quiet==0):
            print "  Addr %X == %04X" % (addr,theresult)
        return theresult
    ...
    def check_skl_round_function(self):
        # Good output looks like this:
        #Simulation results (Good data)
        #Key / Data / state :0000 0000 0000 0000 0000 0000 0000 0000 / 0000 0000 0000 0000 0000 0000 0000 0000 / 02
        #Key / Data / state :6363 6367 6363 6367 6363 6367 6363 6367 / 6363 6363 6363 6363 6363 6363 6363 6363 / 03
        #Key / Data / state :85FB FBF3 E698 9894 85FB FBF3 E698 9894 / 9898 989C 9898 989C 9898 989C 9898 989C / 04
        #Key / Data / state :0D97 0F1F 886C F4EC 6EF4 6C78 EB0F 978B / 7025 259E 1346 46F9 7025 259E 1346 46F9 / 05
        self.set_mode(fips=0,opmode=7)
        self.tp_step(count=1, quiet=1)
        self.tp_enter()

        aes_base = 0x200
        for reg in range(8): # Clean key and data
            self.tp_write(aes_base+reg,0) 
            self.tp_write(aes_base+0x10+reg,0)
        s.tp_write(0x221,2)  # Set to round = 2
        ...
        for i in range(4):
            ...
            for j in range(8):
                data[7-j]=(self.tp_read(0x210+j,quiet=1))
                key[7-j]=(self.tp_read(0x200+j,quiet=1))
                state = self.tp_read(0x221,quiet=1) & 0xf
            astr ="Key / Data / state :"
            for j in range(8):
                astr += "%04X " % key[j]
            astr += "/ "
